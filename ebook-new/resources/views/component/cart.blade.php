        ...
        @if (count(\Cart::getContent()) > 0)
            ...
            <table class=" w-full shadow-inner my-20">
                    ...
                    @foreach (\Cart::getContent() as $item)
                        <tr>
                            <td class="p-4 grid place-content-center ">
                                <img class="h-56" src="{{ asset('storage/books/' . $item->associatedModel->image) }}"
                                    alt="{{ $item->name }}">
                            </td>
                            ...
                            <td class="p-4 text-center">
                                <div>
                                    <button class="px-4 shadow"><a
                                            href="{{ route('cart.decrease', ['item' => $item->id]) }}">-</a></button>
                                    <input type="text" name="qty" value="{{ $item->quantity }}"
                                        class="w-16 text-2xl text-center bg-[#e6ddc4] text-[#6d4b2f] outline-none" />
                                    <button class="px-4 shadow"><a
                                            href="{{ route('cart.increase', ['item' => $item->id]) }}">+</a></button>
                                </div>
                            </td>
                            <td class="p-4 px-6 text-center text-3xl text-[#6d4b2f]">
                ...
                <tfoot class="bg-[#e6ddc4] text-[#6d4b2f]">
                    ...
                    <td class="p-2 float-right text-3xl"><strong>{{ number_format(\Cart::getTotal(), 2, '.', '') }}
                            &euro;</strong></td>
                </tfoot>
            </table>
            @auth
                <div class="flex justify-end mt-4 p-5">
                    <a href="{{ route('checkout') }}"
                        class="px-6 py-3 text-[#6d4b2f] font-semibold bg-[#e6ddc4] hover:text-[#e6ddc4] hover:bg-[#6d4b2f] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg text-center">
                        Checkout
                    </a>
                </div>
            @endauth
            @guest
                ...
                <a href="{{ route('login') }}"
                    class="inline-block rounded-xl py-3 px-5 text-[#6d4b2f] font-semibold bg-[#e6ddc4] hover:text-[#e6ddc4] hover:bg-[#6d4b2f] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium text-lg">Please
                    Login first to checkout </a>
